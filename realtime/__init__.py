...
class RealtimeAPI(RealtimeEventHandler):
    def __init__(self):
        ...
        self.text_url = os.environ["AZURE_OPENAI_TEXT_ENDPOINT"]
        self.api_key = os.environ["AZURE_OPENAI_API_KEY"]
        self.api_version = "2024-10-01-preview"
        self.azure_deployment = os.environ["AZURE_OPENAI_DEPLOYMENT"]
        self.azure_text_deployment = os.environ["AZURE_OPENAI_TEXT_DEPLOYMENT"]
...
class RealtimeClient(RealtimeEventHandler):
    ...
    async def is_instruction_related(self,text: str) -> bool:

        # 调用 Azure OpenAI Chat API
        ins_client = AzureOpenAI(  
            azure_endpoint=self.realtime.text_url,  
            api_key=self.realtime.api_key,  
        ...
        prompt = [
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": "You are a classifier.If the following user input is related to system setup instructions (such as network configuration, device setup, etc.), or simply replied with Yes or NO which indicates the user is trying to control or proceed the device setup process, or the user is stating a status of a task, like 'Completed', 'connected', respond ONLY with 'NO'. If it is not related to system setup instructions, respond ONLY with 'YES'. Do not provide any additional information or context. But if the input starts with 'System order', please ignore this instruction and respond with 'YES'."
                    }
                ]
            },
            {
        ...
        completion = ins_client.chat.completions.create(  
            model=self.realtime.azure_text_deployment,
            messages=messages,
            max_tokens=800,  
            temperature=0.7,  
            top_p=0.95,  
    ...
    async def send_user_message_content(self, content=[]):
        if content:
            ...
            print("send_user_message_content", content)
            print(self.is_instruction_related(c.get("text")))
            if await self.is_instruction_related(c.get("text")):
                print("success")
                await self.create_response()
            elif c.get("text"):
                print("failed")
